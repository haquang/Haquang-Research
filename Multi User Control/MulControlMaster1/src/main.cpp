...
#include <iostream>
#include <fstream>
#define PC_Master_ON 0
#define PC_Slave_ON 0
#define Slave_ON 1
#define Vs_min 50
...
void SlaToMas1_cb(const geometry_msgs::PointStampedConstPtr &msg)
{
	...
	for(i=0; i < DT-1; i++)
	{
		...
		SlaToMas1_pack.Fk[i] = SlaToMas1_pack.Fk[i+1];
...
HDCallbackCode HDCALLBACK omni_callback(void *pUserData)
{
...
#if (PC_Slave_ON)
	{
		...
		{
			sla_mst1_cmd_N +=Fk*Vm1; // backward 1 step
			Xs = Xs - Vs;
			//Modify Vs
			if (Vm1*Vm1>0)
				Fk = (sla_mst1_cmd_P+sla_mst1_cmd_N)/Vm1;
			else
				Fk=0;
	...
	ROS_INFO("Xm - Xs - Force: %5f  %5f  %5f",X1,Xs,F1);
