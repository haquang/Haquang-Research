...
void SlaToMas2_cb(const geometry_msgs::PointStampedConstPtr &msg)
{
	...
	for(i=0; i < DT-1; i++)
	{
		...
		SlaToMas2_pack.Vs[i] = SlaToMas2_pack.Vs[i+1];
...
HDCallbackCode HDCALLBACK phantom_callback(void *pUserData)
{
	...
	{
		sla_mst2_cmd_N +=Fk*Vs; // backward 1 step
		//Modify Vs
		if (Fk*Fk>0)
			Vs = (sla_mst2_cmd_P+sla_mst2_cmd_N)/Fk;
		else
			Vs=0;
